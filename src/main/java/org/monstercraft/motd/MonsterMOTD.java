...
public class MonsterMOTD extends JavaPlugin implements Listener {
    ...
    private class Update implements Runnable {
        ...
        public void run() {
            try {
                final URL url = new URL("http://dev.bukkit.org/server-mods/monstermotd/files.rss");
                ...
                if (firstNode.getNodeType() == 1) {
                    ...
                    if (cur.contains(this.version)) {
                        ...
                        MonsterMOTD.this.log(cur + " is out! You are running " + this.version);
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if ((args.length > 1) && args[0].equalsIgnoreCase("clear")) {
            ...
            this.setMOTD(null);
            sender.sendMessage(ChatColor.GREEN + "MOTD successfully set back to the default: " + ChatColor.RESET + this.defaultMOTD);
            return true;
        }
        if ((args.length > 1) && args[0].equalsIgnoreCase("set")) {
            ...
            this.setMOTD(motd);
            sender.sendMessage(ChatColor.GREEN + "MOTD successfully set to: " + ChatColor.RESET + motd);
            return true;
        }
        // Else, display motd.
        if (sender.hasPermission("monstermotd.view")) {
            sender.sendMessage(ChatColor.GREEN + "Todays MOTD: " + ChatColor.RESET + this.currentMOTD);
        }
        sender.sendMessage(this.DENIED);
        return true;
    }
